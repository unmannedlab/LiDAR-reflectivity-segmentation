...
class GaussianLoss(torch.nn.Module):
    ...
    def forward(self, pred, target,labels):
        masks = torch.nn.functional.one_hot(labels, num_classes=self.num_classes).permute(0,3,1,2).float()
        loss = 0
        count = 0
        for i in range(self.num_classes):
            if i == 0 or masks[:,i].sum() == 0:
                continue
            mask = masks[:,i].unsqueeze(1)
            pred_i = pred*mask
            target_i = target*mask
            var = target_i.var()*torch.ones_like(target_i).to(target_i.device)
            loss += self.gnl(pred_i,target_i,var)
            count += 1
        return loss/count
...
class Trainer():
    def __init__(self, ARCH, DATA, datadir, logdir, path=None,uncertainty=False):
        ...
        intensity_loss_type = self.ARCH["train"].get("intensity_loss", "l1")
        if intensity_loss_type == "l1":
            self.intensity_crit = nn.L1Loss()
        elif intensity_loss_type == "l2":
            self.intensity_crit = nn.MSELoss()
        elif intensity_loss_type == "gaussian":
            self.intensity_crit = GaussianLoss(self.parser.get_n_classes())
        self.intensity_loss_type = intensity_loss_type
        self.intensity_weight = self.ARCH["train"].get("intensity_weight", 0.1)
        print("Intensity loss type: ", self.intensity_loss_type)
        print("Intensity loss weight: ", self.intensity_weight)
        self.ls = Lovasz_softmax(ignore=0).to(self.device)
        self.SoftmaxHeteroscedasticLoss = SoftmaxHeteroscedasticLoss().to(self.device)
        # loss as dataparallel too (more images in batch)
        if self.n_gpus > 1:
            self.criterion = nn.DataParallel(self.criterion).cuda()  # spread in gpus
            self.intensity_crit = nn.DataParallel(self.intensity_crit).cuda()
        ...
        #self.optimizer._scale_fn_custom = self.optimizer._scale_fn_ref()
        #self.optimizer._scale_fn_ref = None
        ...
        if self.path is not None:
            ...
            #prefix = 'module.'
            #n_clip = len(prefix)
            #f_dict = {k[n_clip:]: v for k, v in w_dict['state_dict'].items() if k.startswith(prefix)}
    ...
    def train_epoch(self, train_loader, model, criterion, optimizer, epoch, evaluator, scheduler, color_fn, report=10,
        ...
        for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, proj_near_range, _, _) in enumerate(train_loader):
            ...
            if self.gpu:
                ...
                proj_near_range = proj_near_range.cuda()
                proj_mask = proj_mask.cuda()


            # compute output
            if self.model_name == "SalsaNextAdf":
                output = model(in_vol)
                ...
                output = output_mean
            elif self.model_name == "SalsaNext":
                output = model(in_vol)
                loss_m = criterion(torch.log(output.clamp(min=1e-8)), proj_labels.long()) + self.ls(output, proj_labels.long())
            elif self.model_name in ["SalsaNextLate","SalsaNextEarly"]:
                output,intensity_output = model(in_vol)
                loss_m = criterion(torch.log(output.clamp(min=1e-8)), proj_labels.long()) + self.ls(output, proj_labels.long())
                if self.intensity_loss_type=="gaussian":
                    intensity_loss = self.intensity_crit(intensity_output,proj_near_range,proj_labels)
                else:
                    intensity_loss = self.intensity_crit(intensity_output*proj_mask,proj_near_range*proj_mask)
                loss_m += self.intensity_weight*intensity_loss
            else:
                raise NotImplementedError(f"Model {self.model_name} not implemented")
    ...
    def validate(self, val_loader, model, criterion, evaluator, class_func, color_fn, save_scans):
        ...
        with torch.no_grad():
            ...
            for i, (in_vol, proj_mask, proj_labels, _, path_seq, path_name, _, _, _, _, _, _, proj_near_range, _, _) in enumerate(val_loader):
                if not self.multi_gpu and self.gpu:
                    in_vol = in_vol.cuda()
                    proj_mask = proj_mask.cuda()
                if self.gpu:
                ...
                if self.model_name == "SalsaNextAdf":
                    ...
                    if self.intensity_loss_type=="gaussian":
                        ...
                        intensity_loss = self.intensity_crit(intensity_output*proj_mask,proj_near_range*proj_mask)
                    loss = wce + jacc + intensity_loss*self.intensity_weight
                else:
                    raise NotImplementedError(f"Model {self.model_name} not implemented")              
