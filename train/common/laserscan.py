...
class LaserScan:
    ...
    def reset(self):
        ...
        self.reflectivity = np.zeros((0, 1), dtype=np.float32)
        self.near_range = np.zeros((0, 1), dtype=np.float32)
        ...
        self.proj_remission = np.full((self.proj_H, self.proj_W), -1,
                                      dtype=np.float32)
        self.proj_reflectivity = np.full((self.proj_H, self.proj_W), -1,
                                        dtype=np.float32)
        self.proj_near_range = np.full((self.proj_H, self.proj_W), -1,
                                        dtype=np.float32)
    ...
    def open_scan(self, filename):
        ...
        if self.drop_points is not False:
            ...
            reflectivity = np.delete(reflectivity,self.points_to_drop)
            near_range = np.delete(near_range,self.points_to_drop)

        self.set_points(points, remissions,reflectivity,near_range)

    def set_points(self, points, remissions=None,reflectivity=None,near_range=None):
        ...
        if reflectivity is not None and not isinstance(reflectivity, np.ndarray):
            raise TypeError("Reflectivity should be numpy array")
        if near_range is not None and not isinstance(near_range, np.ndarray):
            raise TypeError("Near range should be numpy array")
        ...
        if reflectivity is not None:
            ...
            self.reflectivity = np.zeros((points.shape[0]), dtype=np.float32)
        
        if near_range is not None:
            self.near_range = near_range
        else:
            self.near_range = np.zeros((points.shape[0]), dtype=np.float32)
    ...
    def do_range_projection(self):
        ...
        reflectivity = self.reflectivity[order]
        near_range = self.near_range[order]
        proj_y = proj_y[order]
        proj_x = proj_x[order]
        ...
        self.proj_reflectivity[proj_y, proj_x] = reflectivity
        self.proj_near_range[proj_y, proj_x] = near_range
