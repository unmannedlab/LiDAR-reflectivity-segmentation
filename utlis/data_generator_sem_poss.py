...
def convert_ply2bin(bin_path):
    ...
    #print('lol')
    points = load_bin('/media/usl/Data/Dataset/Semantic-poss/SemanticPOSS_dataset/dataset/sequences/'+dir_index+'/velodyne/'+bin_path)
    labels = np.fromfile('/media/usl/Data/Dataset/Semantic-poss/SemanticPOSS_dataset/dataset/sequences/'+dir_index+'/labels/'+bin_path[:-3]+'label',dtype = np.int32,count = -1)
    ...
    cal_intensity = ((ins*rang/250)-1)#/np.cos(angles)
    # if np.max(cal_intensity)>500:
    #     print('lol')
    print(np.max(cal_intensity),np.max(rang))
    ...
    points_f[:,0], points_f[:,1], points_f[:,2], points_f[:,3], points_f[:,4], points_f[:,5] = x,y,z, intensity/255, cal_intensity, cal_intensity
    spherical_projection.main(points_f)
    #spherical_projection.main(points_n)
    points_f.tofile('/media/usl/Data/Dataset/Semantic-poss/semantic_poss_reflectivity/sequences/'+dir_index+'/velodyne/'+bin_path)
    labels.tofile('/media/usl/Data/Dataset/Semantic-poss/semantic_poss_reflectivity/sequences/'+dir_index+'/labels/'+bin_path[:-3]+'label')
    #print(bin_path)
    #return x, y, z, cal_intensity


if __name__ =="__main__":
    files_dir= ['00','01','02','03','04','05']
    #fit = np.load('./eta_fit_grass.npy',allow_pickle = True)
    #p = np.poly1d(fit)
    for dir_index in files_dir:
        print('Started Sequence:',dir_index)
        files = os.listdir('/media/usl/Data/Dataset/Semantic-poss/SemanticPOSS_dataset/dataset/sequences/'+dir_index+'/velodyne')
